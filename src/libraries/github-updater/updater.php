...
class WP_GitHub_Updater {
    ...
    /**
     * @var array $missing_config any config that is missing from the initialization of this
     *     instance
     * @access public
     */
    ...
    /**
     * @var array $github_data temporarily store the data fetched from GitHub, allows us to only
     *     load the data once per class instance
     * @access private
     */
    private $github_data;
     ...
     * @param object $response plugin arguments
     *
     * @return bool|object $response the plugin info
     */
     ...
     * @return bool
     */
    protected function has_minimum_config() {
        $this->missing_config = [];
        ...
        foreach ( $required_config_params as $required_param ) {
            if ( ! isset( $this->config[ $required_param ] ) ) {
    ...
    protected function set_defaults() {
        if ( ! empty( $this->config['access_token'] ) ) {
            ...
            list( $scheme, $host, $path ) = parse_url( $this->config['zip_url'] ); // $scheme, $host, $path
    ...
    protected function get_new_version() {
        ...
        if ( $this->overrule_transients() || empty( $version ) ||
            ...
            if ( ! empty( $version ) ) {
                set_site_transient( $this->config['slug'] . '_new_version', $version, ( HOUR_IN_SECONDS * 6 ) );
            }
        }
    ...
    protected function remote_get( $url ) {
        ...
        if ( ! empty( $this->config['access_token'] ) ) {
            $url = add_query_arg( [ 'access_token' => $this->config['access_token'] ], $url );
        }
     ...
     *
     * @since 1.0
     * @return bool|array $github_data the data
     */
    protected function get_github_data() {
        if ( isset( $this->github_data ) && ! empty( $this->github_data ) ) {
            ...
            if ( $this->overrule_transients() || ( ! isset( $github_data ) || ! $github_data || '' == $github_data ) ) {
                ...
                // refresh every 6 hours
                set_site_transient( $this->config['slug'] . '_github_data', $github_data, ( HOUR_IN_SECONDS * 6 ) );
            }
    ...
    protected function get_date() {
        ...
        return ( ! empty( $_date->updated_at ) ) ?
            date( 'Y-m-d', strtotime( $_date->updated_at ) ) : false;
    }
    ...
    /**
     * Check wether or not the transients need to be overruled and API needs to be called for every
     * single page load
     *
     * @access private
